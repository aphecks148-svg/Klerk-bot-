...
const C={
1:["balance","deposit","withdraw","transfer","daily","weekly","monthly","work","job","jobs","salary","beg","invest","loan","credit","business","property","networth","tax","payday","bonus","cashdrop","bankinterest","savings","vault","money","richest","economy"],
2:["shop","buy","sell","inventory","use","trade","market","auction","items","prices","iteminfo","materials","storage","stash","gift","give","exchange","bundle","restock","rarity","blueprint","salvage","craft","collection","stashinfo","sellall","marketvalue","restockinfo","marketquest","itemdrop","itemsearch","itemcount","materialvalue","markethelp"],
...
built.menu=async({reply,args,user,group})=>{
  ...
  s+=`help <cmd> search <word> ${pg===1?"menu 2":"menu 1"}`;
  reply(s);
};
built.commands=async({reply,group})=>reply([...commands.keys()].sort().map(x=>group.prefix+x).join(" • "));
built.help=async({reply,args,group})=>{const n=norm(args[0]);reply(n&&commands.has(n)?`${group.prefix}${n} from ${commands.get(n).file}`:`Use ${group.prefix}menu 1 / menu 2`)};
built.search=async({reply,args,group})=>{const q=norm(args.join(" ")),x=[...commands.keys()].filter(n=>n.includes(q));reply(x.length?x.map(n=>group.prefix+n).join(" • "):"No commands")};
...
async function dispatch(api,event){
  ...
  try{
    await commands.get(n).run({
      ...
      profile:id=>getProfile(api,id),reply:send,
      fun:{fmt,formatNumber:fmt,random:(a,b)=>Math.floor(Math.random()*(b-a+1))+a,isAdmin:id=>isAdmin(id,getUser(id)),gemini:q=>askGemini(q,u),askGemini:q=>askGemini(q,u),canvas:Canvas}
    });
  }catch(e){console.log(n,e.stack||e);await send(`${PREFIX}${n} error ${e.message}`)}
}
...
function loginBot(){
  ...
  login({appState:state},(err,api)=>{
    ...
    try{api.setOptions({listenEvents:true,selfListen:false,updatePresence:true})}catch{}
