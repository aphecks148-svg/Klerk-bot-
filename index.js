// index.js - KILLER BOT 400 CMDS - RENDER READY - FINAL
// Owner: Ahmed Khan ID 100086783504073
// Stack: ws3 fca-priyansh + express + canvas + axios + render port
...
app.get('/', (req,res)=> res.send(`Killer Bot 400 cmds Online! Owner ${process.env.OWNER_ID || "100086783504073"} - ${new Date()} - Commands 400 loaded`));
app.listen(PORT, ()=> console.log(`Render Port ${PORT} listening!`));

const PREFIX = "!";
const OWNER_ID = process.env.OWNER_ID || "100086783504073";
const GEMINI_KEY = process.env.GEMINI_KEY || ""; // Must start with AQ... e.g AQAbc...
const APPSTATE = process.env.APPSTATE ? JSON.parse(process.env.APPSTATE) : JSON.parse(fs.readFileSync('./appstate.json','utf8'));
...
// ===== ALL LISTS IN CODE - EVERY LIST MENTIONED! =====
const PETS_LIST = [
...
const WORKS_LIST = [
  { name:"Farm", earn:"$50k", xp:100, emoji:"🌾" },
  { name:"Mine", earn:"$50k", xp:100, emoji:"⛏️" },
  { name:"Fish", earn:"$30k", xp:80, emoji:"🎣" },
  { name:"Hunt", earn:"$40k", xp:90, emoji:"🏹" },
  { name:"Beg", earn:"$5k", xp:20, emoji:"🪓" },
  { name:"Work", earn:"$20k", xp:50, emoji:"💼" }
];

const MISSIONS_LIST = Array.from({length:30}, (_,i)=>({ id:i+1, name:`Mission ${i+1}`, req:`Level ${i*10+1}`, reward:`$${(i+1)*100000}+XP ${(i+1)*100}+Pet`, emoji:"🎯" }));

// ===== CANVAS BOX GENERATOR WITH BARS =====
async function genBox(title, userName, userPicUrl, stats){
  ...
  // Bar
  if(stats.includes("%")){
    const pct = parseInt(stats.match(/(\d+)%/)?.[1]||50);
    ctx.fillStyle = "#444"; ctx.fillRect(20,150,300,30);
    ctx.fillStyle = "#0f0"; ctx.fillRect(20,150,300*pct/100,30);
  }
...
function parseCmd(text){
  if(!text) return null; let t=text.trim(); if(!t.startsWith(PREFIX)) return null;
  t=t.slice(1).trim(); if(!t) return null; // SPACE SUPPORT: "! balance" -> "balance"
  const parts=t.split(/\s+/); return {cmd:parts[0].toLowerCase(), args:parts.slice(1)};
}
async function geminiAQ(q){
  try{
    if(!GEMINI_KEY.startsWith("AQ")) return `⚠️ Gemini key must start with AQ! Got ${GEMINI_KEY.slice(0,2)}...`;
  ...
  }catch(e){ return `🤖 AQ Bot: You said "${q}" - I am Killer Bot 400 cmds! Owner Ahmed ${OWNER_ID}! Type!menu!`; }
}

// LOAD 8 CMD FILES - 50 EACH = 400
let commands={};
const files=["economy.js","pets.js","games.js","info.js","utility.js","ai_media.js","admin.js","owner.js"];
for(let f of files){
  const fp=path.join(__dirname,"cmds",f);
  if(fs.existsSync(fp)){
    const mod=require(fp);
    for(let k in mod) commands[k.toLowerCase()]=mod[k];
  }
}
console.log(`Loaded ${Object.keys(commands).length} cmds - Lists: Pets ${PETS_LIST.length}, Weapons ${WEAPONS_LIST.length}, Cars ${CARS_LIST.length}, Seeds ${SEEDS_LIST.length}, Crypto ${CRYPTO_LIST.length}, Pokemon ${POKEMON_LIST.length}, Crates ${CRATES_LIST.length}`);
...
function spawnPokemon(api){
  ...
  for(let tid in data.threads){
    api.sendMessage(`╭─〔 🐾 WILD POKEMON SPAWNED! 🐾 〕─╮\n${poke.emoji} ${poke.name} Lv${Math.floor(Math.random()*100)+1} appeared!\n💰 Worth $${poke.worth.toLocaleString()} Power ${poke.power} Rarity ${poke.rarity}!\nType!catch to catch!\nNext in 20 min!`, tid);
  }
}

// NEVER SLEEP + AUTO REACTS
function neverSleep(api){
  setInterval(()=>{ save(); for(let tid in data.threads) api.markAsRead(tid,true); },30000);
  setInterval(()=> spawnPokemon(api), 20*60*1000);
  console.log("Never Sleep ON - Auto Seen 30s - Pokemon 20min - Auto Reacts ON");
}
...
login({appState: APPSTATE}, (err,api)=>{
  ...
  neverSleep(api);
  console.log("BOT ONLINE 400 CMDS");
  ...
  api.listenMqtt(async (err,event)=>{
    ...
    try{
      ...
      // AUTO REACT TO EVERY MSG!
      if(event.type=="message" && event.messageID){
        const emojis=["🔥","❤️","😂","😮","👍","🎉","💯","⚡","🌌","👑"];
        api.setMessageReaction(emojis[Math.floor(Math.random()*emojis.length)], event.messageID, ()=>{}, true);
      }
      if(event.type=="message" && event.body){
        const parsed=parseCmd(event.body);
        if(parsed && commands[parsed.cmd]){
          if(user.banned && uid!=OWNER_ID) return api.sendMessage(`🚫 Banned! Contact Owner ${OWNER_ID}`,tid);
          if(thread.onlyAdmin && !thread.whitelist.includes(uid) && uid!=OWNER_ID) return api.sendMessage(thread.onlyAdminText,tid);
          await commands[parsed.cmd]({ api, event, args:parsed.args, user, thread, data, uid, tid, OWNER_ID, makeBar, addXP, getUser, genBox, PETS_LIST, WEAPONS_LIST, CARS_LIST, SEEDS_LIST, CRYPTO_LIST, CRATES_LIST, POKEMON_LIST, currentPokemon, save, geminiAQ, POKEMON_LIST_REF: POKEMON_LIST });
        }else if(!parsed){
          // BOT REPLY DIRECTLY TO EVERY MSG + GEMINI AQ!
          if(event.body.length>1){
            const aq=await geminiAQ(event.body);
            const canvasBuf=await genBox(`AQ Reply`, event.senderID, null, `${event.body.slice(0,20)} - Level ${user.level} ${makeBar(user.xp, user.level*1000)}`);
            api.sendMessage({body:`╭─〔 🤖 AQ - ${uid} - LVL ${user.level} ${makeBar(user.xp, user.level*1000,8)} 〕─╮\n👤 You: ${event.body}\n🤖 AQ: ${aq}\n💰 Cash $${user.cash.toLocaleString()} Bank $${user.bank.toLocaleString()}\n📊 Level ${user.level}/1000 Prestige ${user.prestige} XP ${user.xp}\nType!menu for 400 cmds!\n╰──────────────────╯`, attachment:[]}, tid);
          }
        }
      }
...
module.exports = { PETS_LIST, WEAPONS_LIST, CARS_LIST, SEEDS_LIST, CRYPTO_LIST, CRATES_LIST, POKEMON_LIST, WORKS_LIST, MISSIONS_LIST };
