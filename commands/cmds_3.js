const U=new Map();
const u=id=>{
 if(!U.has(id))U.set(id,{cash:1000000,bank:0,vault:0,savings:0,inv:[],salary:25000,credit:500,loan:0,business:[],stocks:{},crypto:{},properties:[],market:[]});
 return U.get(id);
};
const r=(c,s)=>c.reply(`╭━━ 💰 ECONOMY ━━╮\n${s}\n╰━━━━━━━━━━━━━━╯`);
const cmd=(name,description,execute,aliases=[])=>({name,description,execute,aliases,reaction:"💰"});
...
C.daily=cmd("daily","Daily game money reward",c=>{const x=u(c.event.senderID);x.cash+=6000000;r(c,`🎁 DAILY REWARD\n💰 +$6,000,000\n💵 Wallet: ${money(x.cash)}`)});
C.weekly=cmd("weekly","Weekly money reward",c=>{const x=u(c.event.senderID);x.cash+=20000000;r(c,"🎁 WEEKLY REWARD\n💰 +$20,000,000")});
C.monthly=cmd("monthly","Monthly money reward",c=>{const x=u(c.event.senderID);x.cash+=50000000;r(c,"🎁 MONTHLY REWARD\n💰 +$50,000,000")});

C.shop=cmd("shop","Open economy shop",c=>r(c,"🛒 SHOP\n🍎 Food\n🛠️ Tools\n🎒 Items\n💎 Rare items\n🏠 Property items\n🚗 Vehicle items"));
C.inventory=cmd("inventory","View inventory",c=>{
 const x=u(c.event.senderID);
 r(c,x.inv.length?x.inv.map((v,i)=>`${i+1}. ${v}`).join("\n"):"🎒 Inventory empty.");
},["inv"]);

C.salary=cmd("salary","Collect salary",c=>{const x=u(c.event.senderID);x.cash+=x.salary;r(c,`💼 Salary: ${money(x.salary)}`)});
C.paytaxes=cmd("paytaxes","Pay fictional game taxes",c=>{const x=u(c.event.senderID),n=Math.min(x.cash,100000);x.cash-=n;r(c,`🏛️ Taxes paid: ${money(n)}`)});
C.vault_lock=cmd("vault_lock","Lock your vault",c=>r(c,"🔐 Vault locked. Extra protection enabled."));
C.loan=cmd("loan","View or request game loan",c=>{const x=u(c.event.senderID);r(c,`🏦 Loan balance: ${money(x.loan)}\n💳 Credit score: ${x.credit}`)});
...
C.audit=cmd("audit","View personal financial audit",c=>{const x=u(c.event.senderID);r(c,`📊 FINANCIAL AUDIT\n💵 Cash ${money(x.cash)}\n🏦 Bank ${money(x.bank)}\n🔐 Vault ${money(x.vault)}\n🏠 Properties ${x.properties.length}`)});
...
].forEach(n=>C[n]=cmd(n,"Business system",c=>{
 const x=u(c.event.senderID);
 if(n==="business_buy"){x.business.push(c.args.join("_")||"Starter Business");return r(c,"🏢 Business purchased.")}
 if(n==="business_sell"){if(x.business.length)x.business.pop();return r(c,"💵 Business sold.")}
...
[
"stock_buy","stock_sell","stock_portfolio","stock_market","stock_vanguard","stock_tesla",
"stock_apple","stock_google","stock_amazon","stock_meta","stock_nvidia","stock_microsoft",
"stock_spacex","stock_cyberdyne","stock_starkcorp","stock_wayneent","stock_umbrella",
"stock_robocorp","stock_insider","stock_options"
].forEach(n=>C[n]=cmd(n,"Fictional stock market",c=>{
 const x=u(c.event.senderID),s=n.replace("stock_","");
 if(n==="stock_market")
  return r(c,"📈 FICTIONAL STOCK MARKET\n"+stocks.map(a=>`• ${a.toUpperCase()} — $${100+Math.floor(Math.random()*900)}`).join("\n"));
 if(n==="stock_portfolio")
  return r(c,`📊 Portfolio:\n${Object.keys(x.stocks).length?Object.entries(x.stocks).map(([a,b])=>`${a}: ${b}`).join("\n"):"Empty"}`);
...
[
"crypto_buy","crypto_sell","crypto_wallet","crypto_exchange","crypto_btc","crypto_eth",
"crypto_sol","crypto_doge","crypto_usdt","crypto_citycoin","crypto_darkcoin","crypto_nft",
"crypto_stake","crypto_rig","crypto_ico","crypto_rugpull","crypto_airdrop","crypto_futures",
"crypto_whale","crypto_hack"
].forEach(n=>C[n]=cmd(n,"Fictional crypto game system",c=>{
 const x=u(c.event.senderID);
 if(n==="crypto_wallet")
  return r(c,Object.keys(x.crypto).length?Object.entries(x.crypto).map(([a,b])=>`${a}: ${b}`).join("\n"):"🪙 Empty crypto wallet.");
 r(c,`🪙 ${n.replace(/_/g," ").toUpperCase()}\n🎮 Fictional game economy action completed.`);
}));

// 🏪 MARKET / TRADING
// market_buy is owned HERE; removed from cmds_2.js.
[
...
].forEach(n=>C[n]=cmd(n,"Market and trading system",c=>{
...
"buy_furniture","place_furniture","remove_furniture","house_party","guest_list","neighbor",
"neighborhood_chat","rent","tenant","landlord","property_market","property_tax","home_security","home_alarm"
].forEach(n=>C[n]=cmd(n,"Housing and life system",c=>{
 const x=u(c.event.senderID);
